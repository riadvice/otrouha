...
class Exam
{
    ...
    public function index($f3, $params): void
    {
        ...
        if (array_key_exists('help', $_GET)) {
            ...
            foreach ($classes as $class) {
                $name = preg_replace('/[a-z]{1,}\\\|actionstest|test/', '', strtolower($class), -1);
        ...
        CliUtils::instance()->write(PHP_EOL . "--- Exam Starting unit tests at {$f3->get('utest.time')} ---");
        $results = [];

        foreach ($classes as $class) {
            if (!in_array('all', $tests) && !in_array(preg_replace('/[a-z]{1,}\\\|actionstest|test/', '', strtolower($class), -1), $tests)) {
                continue;
            }
            $object = new $class();
        ...
        $f3->set('utest.exam_folder', !$this->cli ? '/exam/' : '');
    ...
    private static function isCoverageEnabled()
    {
        if (self::$coverageEnabled === null) {
            self::$coverageEnabled = array_key_exists('exam', $_GET) && $_GET['exam'] === 'withCoverage';
    ...
    public static function writeCoverageResult(): void
    {
        if (self::isCoverageEnabled()) {
            ...
            $publicFolder = dirname(dirname(dirname(__DIR__))) . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'exam';
            $writer       = new Facade();
    ...
    {
        return $f3->get('ROOT') . DIRECTORY_SEPARATOR . 'exam' . DIRECTORY_SEPARATOR . 'test.result';
    }

    public function afterroute(): void
    {
        \Base::instance()->set('UI', '../tests/ui/');
        $result = \Preview::instance()->render('exam.htm');
        if (!$this->cli) {
            echo $result;
        } else {
