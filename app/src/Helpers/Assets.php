...
class Assets extends Base
{
     ...
     * @return string
     */
    public function renderCssTag($filePath, $id)
    {
        ...
        $idTag = $id ? 'id="' . $id . '"' : '';
        if ($this->f3->get('MINIFY_CSS') == true && stripos($filePath, 'http') === false && !Strings::contains($filePath, '.min.')) {
            $cssTag = '<link href="/minified/' . $this->minifyCSS($filePath, stripos($filePath, '.min.')) . '" rel="stylesheet" type="text/css" ' . $idTag . '/>' . "\n";
        } else {
    ...
    {
        return "Common.setCDN('{$this->f3->get('CDN')}');\n";
    }
    ...
    public function renderJsTag($filePath)
    {
        ...
        if ($this->f3->get('MINIFY_JS') == true && !Strings::contains($filePath, '.min.')) {
            $jsTag = '<script src="/minified/' . $this->minifyJavaScript($filePath, stripos($filePath, '.min.')) . '" type="text/javascript"></script>' . "\n";
        } else {
            V::exists()->check($this->f3->get('ROOT') . $filePath);
